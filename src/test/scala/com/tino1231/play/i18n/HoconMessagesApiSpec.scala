package com.tino1231.play.i18n
...
class HoconMessagesApiSpec extends PlaySpec with GuiceOneAppPerSuite {
  ...
  lazy val lang_pt = Lang("ja")

  implicit override lazy val app = new GuiceApplicationBuilder()
    .configure(Map("play.i18n.langs" -> Seq("en", "ja")))
  ...
  "Hocon Messages API" must {
    ...
    "get messages using the preferred language" in {
      val messagesApi = app.injector.instanceOf[MessagesApi]
      val messages = messagesApi.preferred(Seq(Lang("ja")))

      implicit val lang = lang_pt

      messages("test.messages.simple") mustBe "こんにちは"
      messages("test.messages.parameters", "Stranger") mustBe "こんにちは, Stranger"
    }
